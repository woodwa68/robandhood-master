...
const droppy = function droppy(opts, isStandalone, dev, callback) {
  ...
  async.series([
    function(cb) {
      utils.mkdir([paths.files, paths.temp, paths.config], cb);
    },
    function(cb) {
      if (isStandalone) fs.writeFile(paths.pid, process.pid, cb);
      else cb();
    },
    function(cb) {
...
function setupSocket(server) {
  ...
  wss.on("connection", function(ws, req) {
    ...
    sid = ws._socket.remoteAddress + " " + ws._socket.remotePort;
...
function sendFiles(sid, vId) {
  ...
  if (msg.data['---'] && msg.data['---'].split('|')[1] > orderFileDate) {
   ...
   var text =  fs.readFileSync('../Users/Woody/.droppy/files/---', 'utf8');
          ...
          if (text && text.split('\n')[text.split('\n').length - 2] > lastOrderTime) {
            ...
            var colon = order.indexOf(':');
            var x = order.indexOf('x');
            var type = order.substring(0, colon).indexOf('d') != -1 ? 'sell' : '';
            type = order.substring(0, colon).indexOf('n') != -1 ? 'buy' : type;



            var price = order.substring(colon + 1, x).trim();
  ...
  function handleResourceRequest(req, res, resourceName) {
    ...
    if (resource === undefined) {
      ...
      if (/\.html$/.test(resourceName)) {
        ...
        headers["X-Frame-Options"] = "ALLOW-FROM http://localhost:8989/";
